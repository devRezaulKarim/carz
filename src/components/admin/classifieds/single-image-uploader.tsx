...
export const SingleImageUploader = ({
  ...
  const handleUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_IMAGE_SIZE) {
      setError(`File size exceeds ${convertToMB(MAX_IMAGE_SIZE)} limit`);
      return;
    }
    setError(null);
    setIsUploading(true);
  ...
  return (
    <div className="mx-auto w-full xl:w-2/3 2xl:w-full">
      <div
        className={cn(
          "relative flex aspect-[3/2] cursor-pointer flex-col items-center justify-center rounded-md",
          error && "border-2 border-dotted border-red-500",
        ...
        {preview ? (
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-gray-600">
              Click or drag image to upload (max 2mb)
            </p>
          </div>
        )}
        {isUploading && (
